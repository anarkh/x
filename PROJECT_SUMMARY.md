...
评论存储在 `post_comments`，反馈存储在 `feedback_items`，用户对任务的信任动作存储在本地 `post_reactions` 或云端 `post_reactions`。分享落地和二跳动作的最小归因事件可写入 `viral_attribution_events`，只记录来源、动作、粗粒度状态和链路 id，不记录评论正文、联系人/群或精确经纬度。
...
1. CloudBase 可用时，帖子、评论、反馈、反应、传播归因事件和管理员身份通过云函数处理。
2. 云函数、集合或开发环境不可用时，开发环境会回退到本地 storage 和 mock 数据。

云端相关函数：

- `cloudfunctions/posts`：处理帖子列表、详情、创建、评论、反馈、信任动作、传播归因事件、关闭、隐藏和图片上传路径准备。
